...
export class BudgetSnapshot extends Snapshot<Budget> {
    updateState(e: Event<Budget>, initial: BUDGET_SNAPSHOT<Budget>): BUDGET_SNAPSHOT<Budget> {
        ...
        const updatedBudget = BudgetSnapshot.updateBudget(e, budget)
        return initial.set(email, versions.set(version, updatedBudget))
    }

    static updateBudget(e: Event<Budget>, budget: Budget): Budget {
        switch (e.type) {
            case 'IMPORT_BUDGET':
                return BudgetSnapshot.importBudget(budget, e.payload)
            case 'PUT_ITEM':
                const {payload} = e
                return BudgetSnapshot.importBudget(budget, {[payload.type]: [payload]})
            case 'DELETE_ITEM':
