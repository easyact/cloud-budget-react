...
export class BudgetServiceInterpreter implements BudgetService {
    ...
    getList = (user: string, version: string, name: ItemType): BudgetOp<Option<any>> => asks<BudgetRepository, Task<Option<any>>>(repo => repo.getList(user, version, name))

    setList = (user: string, version: string, name: ItemType, v: any): BudgetOp<void> => asks<BudgetRepository, Task<void>>(repo => repo.setList(user, version, name, v))
